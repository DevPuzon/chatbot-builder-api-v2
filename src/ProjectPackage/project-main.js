...
router.put('/rename',middlewareMain.onCheckProject,async (req,res)=>{
    try{
        const q = req.query;
...
router.delete('/delete',middlewareMain.onCheckProject,async (req,res)=>{
    try{
        const q = req.query;
        var sql = `DELETE FROM chatbot_builder_v2.chatbot_res WHERE project_id = ${SqlString.escape(q.project_id)}; 
        DELETE FROM chatbot_builder_v2.chatbot_res_dep WHERE project_id = ${SqlString.escape(q.project_id)}; 
...
router.post('/duplicate',middlewareMain.isUserValid,async(req,res)=>{
    try{ 
        const b  = req.body;
